...
// need to be skipped before coming to float values
// Postcondition: returns number of rows

void FillArray(ifstream&, Floatpointer&, int line_counter, int array_size, int columnsize, int rows);
...
float MinSearch(const Floatpointer&, int column, int rows);
...
float MaxSearch(const Floatpointer&, int colmn, int rows);
// Precondition: takes array and column number of desired variable
// Postcondition: returns minimum value of that variable from steam table

void Region(const Floatpointer&, int rows, float Tmin, float Tmax, float Pmin, float Pmax);
...
void Psat(const Floatpointer&, int rows, float Pmin, float Pmax);
...
void Tsat(const Floatpointer &, int rows, float Tmin, float Tmax);
// Description: this function does the same thing as "Psat" except the roles of
// T & P are reversed

void Px(const Floatpointer &, int rows, float Pmin, float Pmax);
...
void Tx(const Floatpointer&, int rows, float Tmin, float Tmax);
...
void Ph(const Floatpointer&, int rows, float Pmin, float Pmax);
...
void Th(const Floatpointer&, int rows, float Tmin, float Tmax);
...
void hx(const Floatpointer&, int rows, float hmin, float hmax);
// Precondition: takes array, hmin, and hmax
// Postcondition: returns temperature and pressure

int SearchHigh(const Floatpointer&, int rows, float Var, int column);
...
int SearchLow(const Floatpointer&, int rows, float Var, int column);
// Precondition: takes array, steam table variable, and column # of variable
...
float extrapolate(float x, float yf, float yg);
...
float quality(float yf, float yg, float yav);
...
int main()
{
	int column1 = 1,
		...
		column3 = 3,
		column4 = 4;
	int WrongChoice = 0; // this is to make sure the selection loop iterates if a wrong choice is made
	int array_size;
	int	line_counter;
	int	rows;
	float Tmin, Tmax, Pmin, Pmax, hmin, hmax;
	Floatpointer Array;
	char filename[21];
	char choice[4];
	char again; // this is to help assign value to "answer"

	bool answer, answer1 = true; // these help to end loops
	...
	array_size = ArraySize(input, line_counter); // RETURNS # OF FLOAT VALUES THAT WILL BE READ INTO ARRAY
	input.close();
	...
	hmin = MinSearch(Array, column3, rows);
	hmax = MaxSearch(Array, column3, rows);
	...
	while(answer = true)
	{
		...
		while(WrongChoice == 1)
		{
			...
			else if(choice == "D" || choice == "D." || choice == "d")
			{
				Px(Array, rows, Pmin, Pmax);

				WrongChoice = 2;
			}
			else if(choice == "E" || choice == "E." || choice == "e")
			{
				Tx(Array, rows, Tmin, Tmax);

				WrongChoice = 2;
			}
			else if(choice == "F" || choice == "F." || choice == "f")
			{
				Ph(Array, rows, Pmin, Pmax);

				WrongChoice = 2;
			}
			else if(choice == "G" || choice == "G." || choice == "g")
			{
				Th(Array, rows, Tmin, Tmax);

				WrongChoice = 2;
			}
			else if(choice == "H" || choice == "H." || choice == "h")
			{
				hx(Array, rows, hmin, hmax);
...
int DataStart(ifstream& input)
{
	...
	// STARTING ROW TO DELETE WHEN ARRAY_SIZE WANTS TO FIND THE NUMBER OF VALID FLOATS
	cout << "print 3 in function" << endl;
	char next;
	int line_counter = 0;
    bool var1, var2 = true;
...
void FillArray(ifstream& input, Floatpointer& Array, int line_counter, int array_size, int columnsize, int rows)
{
...
float MinSearch(const Floatpointer& Array, int column, int rows)
{
	int i = 0;
	float minimum = Array[i][column]; // HERE'S THE MINIMUM VARIABLE
...
float MaxSearch(const Floatpointer& Array, int column, int rows)
{
	int i = 0;
	float maximum = Array[i][column]; // HERES THE MAXIMUM VARIABLE
...
void Region(const Floatpointer& Array, int rows, float Tmin, float Tmax, float Pmin, float Pmax)
{
	float T;
	float P;
	float Psat, P2, P1, T2, T1;
	int column1 = 1;
	int column2 = 2;
	int rowhigh, rowlow;
	bool indicator = true;
	...
	{
		cout << "Here you will enter T and P data in order to get info about\n"
		...
		if(T < Tmin && P < Pmin || T > Tmax && P > Pmax)
		{
			...
			cout << "ERROR: You have entered invalid data.\n"
				 << "Try again (Y or N)?";
...
void Psat(const Floatpointer& Array,int rows, float Pmin, float Pmax)
{
	bool indicator = true;
	char response[5];
	int column1 = 1;
	...
	int column6 = 6;
	int rowhigh, rowlow;
	float P, Tsat, hliq, hvap, vliq, vvap;
	float P2, P1, T2, T1, hliq2, hliq1, hvap2, hvap1,
...
void Tsat(const Floatpointer & Array, int rows, float Tmin, float Tmax)
{
	...
	int column6 = 6;
	int rowhigh, rowlow;
	float T, Psat, hliq, hvap, vliq, vvap;
	float P2, P1, T2, T1, hliq2, hliq1, hvap2, hvap1,
		  vvap2, vvap1, vliq2, vliq1;
...
void Px(const Floatpointer & Array, int rows, float Pmin, float Pmax)
{
	bool indicator = true;
	char response[5];
	int rowhigh, rowlow;
	...
	int column5 = 5;
	int column6 = 6;
	float P, Tsat, x, hav, vav, hliq, hvap, vliq, vvap;
	float P2, P1, T2, T1, hliq2, hliq1, hvap2, hvap1,
		  vvap2, vvap1, vliq2, vliq1;
	...
	while(indicator = true)
	{
		...
		else
		{
			...
			cout << "The saturation temperature is: " << Tsat << " C";
			cout << "\n\nThe specific volume is " << vav << " m3/kg\n";
			cout << "\n\nThe enthalpy is " << hav << " kJ/kg\n";
...
void Tx(const Floatpointer& Array, int rows, float Tmin, float Tmax)
{
	bool indicator = true;
	char response[5];
	int rowhigh, rowlow;
	...
	int column5 = 5;
	int column6 = 6;
	float T, Psat, x, hav, vav, hliq, hvap, vliq, vvap;
	float P2, P1, T2, T1, hliq2, hliq1, hvap2, hvap1,
		  vvap2, vvap1, vliq2, vliq1;
...
void Ph(const Floatpointer& Array, int rows, float Pmin, float Pmax)
{
	bool indicator = true;
	char response[5];
	int rowhigh, rowlow;
...
void Th(const Floatpointer& Array, int rows, float Tmin, float Tmax)
{
	bool indicator = true;
	char response[5];
	int rowhigh, rowlow;
...
void hx(const Floatpointer& Array, int rows, float hmin, float hmax)
{
	bool indicator = true;
	char response[5];
	int rowhigh, rowlow;
	...
	int column5 = 5;
	int column6 = 6;
	float h, x, Psat, Tsat, hvap, hliq;
	float hliq2, hliq1, T2, T1, P2, P1;

	while(indicator = true) // THIS WHILE STATMENT IS IN CASE OF ERROR
	{
		cout << "Max h:  " << hmax << " kJ/kg" << endl;
		cout << "Min h:  " << hmin << " kJ/kg" << endl;
		...
		hliq = (h - hvap - x * hvap)/x;
		hvap = (h - hvap * x)/(1+x);

		if(x < 0 || x > 1 || hliq < hmin || hvap > hmax)
		{
			cout << "ERROR: You have entered invalid data.\n"
				 << "Try again (Y or N)?";
		...
		{
			rowhigh = SearchHigh(Array, rows, hliq, column5);
			rowlow = SearchLow(Array, rows, hliq, column5);
			...
			Tsat = interpolate(T2, T1, hliq2, hliq1, h);
			Psat = interpolate(P2, P1, hliq2, hliq1, h);

			cout << "\n\nWith an enthalpy of " << h << " kJ/kg and a quality of " << x << " . . .\n";
			cout << "The saturation temperature is: " << Tsat << " degrees C\n";
			cout << "The saturation pressure is: " << Psat << " degrees C\n";
...
int SearchHigh(const Floatpointer& Array, int rows, float Var, int column)
{
	int ROW;
	for(int i = 1; i < rows; i++)
	{
		if(Array[i - 1][column] < Var && Array[i][column] >= Var)
			ROW = i;
	}
	return ROW;
}

int SearchLow(const Floatpointer& Array, int rows, float Var, int column)
{
	int ROW;
